# PROMPT_AND_PROMPT_GENERATOR_AUDIT_PLAN

## Purpose
Audit approved prompts and the prompt generator logic.

## Scope
Prompt inventory, approved library, generator logic, source basis, scope control, tool/model routing, output requirements, evidence, rollback, stop condition, versioning, reuse, failure register, false-PASS risk, credit-burn risk and customer-data restrictions.

## Audit owner/agent
Prompt Audit Specialist.

## Audit object
Prompts and prompt-generation controls.

## Audit criteria
Prompt generator must select prompt type, insert context, prevent scope drift, block unsafe actions, select agents/skills, apply gate status and stop when evidence is missing.
