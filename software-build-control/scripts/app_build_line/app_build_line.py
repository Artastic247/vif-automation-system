...
def list_automation(root: Path) -> int:
    base = root / "01-app-build-line"
    reports = base / "reports"
    inventory = {
        "timestamp": now_iso(),
        "prompts": sorted([x.name for x in (base / "prompts").glob("*.md")]),
        "commands": sorted([x.name for x in (base / "commands").glob("*.md")]),
        "hooks": sorted([x.name for x in (base / "hooks").iterdir()]) if (base / "hooks").exists() else [],
        "inflows": sorted([x.name for x in (base / "inflows").iterdir()]) if (base / "inflows").exists() else [],
        "schedules": sorted([x.name for x in (base / "schedules").iterdir()]) if (base / "schedules").exists() else [],
        "validators": sorted([x.name for x in (base / "validators").iterdir()]) if (base / "validators").exists() else [],
    }
    write_json(reports / "app-build-automation-inventory.json", inventory)
    md = [
        "# App Build Automation Inventory",
        "",
        f"- timestamp: {inventory['timestamp']}",
        f"- prompts: {len(inventory['prompts'])}",
        f"- commands: {len(inventory['commands'])}",
        f"- hooks files: {len(inventory['hooks'])}",
        f"- inflow files: {len(inventory['inflows'])}",
        f"- schedule files: {len(inventory['schedules'])}",
        f"- validator files: {len(inventory['validators'])}",
    ]
    (reports / "app-build-automation-inventory.md").write_text("\n".join(md) + "\n", encoding="utf-8")
    print(f"created {reports / 'app-build-automation-inventory.json'}")
    print(f"created {reports / 'app-build-automation-inventory.md'}")
    return 0
...
def main() -> int:
    ...
    p_list = sub.add_parser("list-automation")
    p_list.add_argument("--root", required=True)

    args = parser.parse_args()
    root = Path(args.root)

    if args.command == "list-automation":
        return list_automation(root)
